...
class Program
{
    ...
    static bool son_3_herf_check(string input)
    {
        return input.ToLower().GroupBy(c => c).Any(g => g.Count() >= 3);
    }
    static void Main()
    {
        // 2) rhyta.com ve ya dayrep.com domenlerinde emaili olan borclulari cixartmag

        var query2 = debtors.Where(debtor => debtor.Email.EndsWith("@rhyta.com") || debtor.Email.EndsWith("@dayrep.com"));
        Console.WriteLine(" task 2 ");
        foreach (var debtor in query2)
        {
        ...
        var query3 = debtors.Where(debtor => debtor.BirthDay >= DateTime.Now.AddYears(-36) && debtor.BirthDay <= DateTime.Now.AddYears(-26));
        Console.WriteLine(" task3 ");
        foreach (var debtor in query3)
        {
            Console.WriteLine(debtor);
        }
        Console.WriteLine();
        ...
        var query4 = debtors.Where(debtor => debtor.Debt <= 5000);
        Console.WriteLine(" task4 ");
        foreach (var debtor in query4)
        {
            Console.WriteLine(debtor);
        }
        Console.WriteLine();
        ...
        var query5 = debtors.Where(debtor => debtor.FullName.Length > 18 && debtor.Phone.Count(c => c == '2' || c == '7') >= 2);
        Console.WriteLine(" task5 ");
        foreach (var debtor in query5)
        {
            Console.WriteLine(debtor);
        }
        Console.WriteLine();
        ...
        var query7 = debtors.Where(debtor => debtor.BirthDay.Month >= 12 || debtor.BirthDay.Month <= 2);
        Console.WriteLine(" task7 ");
        foreach (var debtor in query7)
        {
            Console.WriteLine(debtor);
        }
        Console.WriteLine();
        ...
        var query8 = debtors.Where(debtor => debtor.Debt > averageDebt).OrderBy(debtor => debtor.FullName).ThenByDescending(debtor => debtor.Debt);
        Console.WriteLine(" task8 :");
        foreach (var debtor in query8)
        {
            Console.WriteLine(debtor);
        }
        Console.WriteLine();
        ...
        var query9 = debtors.Where(debtor => !debtor.Phone.Contains("8"))
                    .Select(debtor => new { debtor.FullName, Age = CalculateAge(debtor.BirthDay), debtor.Debt });

        Console.WriteLine(" task9 ");
        foreach (var debtorInfo in query9)
        {
            Console.WriteLine($"Familya: {debtorInfo.FullName}, Yaş: {debtorInfo.Age}, Ümumi Borc: {debtorInfo.Debt}");
        }
        Console.WriteLine();
        ...
        var query11 = debtors.Where(debtor => son_3_herf_check(debtor.FullName) && son_3_herf_check(debtor.FullName))
                    .OrderBy(debtor => debtor.FullName).ThenBy(debtor => debtor.Debt);

        Console.WriteLine(" task11 ");
        foreach (var debtor in query11)
        {
            Console.WriteLine(debtor);
        }
        Console.WriteLine();
        ...
        var query13 = debtors.GroupBy(debtor => debtor.BirthDay.Year)
                     .OrderByDescending(group => group.Count())
                     .Select(group => new { BirthYear = group.Key, Count = group.Count() })
                     .FirstOrDefault();

        Console.WriteLine(" task13 ");
        if (query13 != null)
        {
            Console.WriteLine($"Ən çox borclu olan il: {query13.BirthYear}, Borclu sayı: {query13.Count}");
        }
        Console.WriteLine();

        // 14) Borcu en boyuk olan 5 borclunun siyahisini cixartmaq

        var query14 = debtors.OrderByDescending(debtor => debtor.Debt).Take(5);

        Console.WriteLine(" task14 ");
        foreach (var debtor in query14)
        {
            Console.WriteLine(debtor);
        }
        Console.WriteLine();
        ...
        var query16 = debtors.Where(debtor => debtor.BirthDay.Year <= 1945);
        Console.WriteLine(" task16 ");
        foreach (var debtor in query16)
        {
            Console.WriteLine(debtor);
        }
        Console.WriteLine();
        ...
        var query18 = debtors.Where(debtor => debtor.Phone.Distinct().Count() == debtor.Phone.Length)
                    .Select(debtor => new { debtor.FullName, debtor.Debt });

        Console.WriteLine(" task18 ");
        foreach (var debtorInfo in query18)
        {
            Console.WriteLine($"Familya: {debtorInfo.FullName}, Ümumi Borc: {debtorInfo.Debt}");
        }
        Console.WriteLine();
        ...
        var query19 = debtors.Where(debtor => debtor.Debt > 0)
                            .Select(debtor => new
                            {
                                debtor.FullName,
                                debtor.Debt,
                                RemainingMonths = Math.Ceiling((debtor.Debt / 500.0))
                            })
                            .OrderBy(debtor => debtor.RemainingMonths);

        Console.WriteLine(" task19 ");
        foreach (var debtorInfo in query19)
        {
            Console.WriteLine($"Familya: {debtorInfo.FullName}, Ümumi Borc: {debtorInfo.Debt}, Qalıq Ay: {debtorInfo.RemainingMonths}");
        }
        Console.WriteLine();
        ...
        var query20 = debtors.Where(debtor => debtor.FullName.ToLower().Intersect("smile").Count() == "smile".Length)
                    .Select(debtor => new { debtor.FullName, debtor.Debt });

        Console.WriteLine(" task20 ");
        foreach (var debtorInfo in query20)
        {
            Console.WriteLine($"Familya: {debtorInfo.FullName}, Ümumi Borc: {debtorInfo.Debt}");
        }
        Console.WriteLine();
